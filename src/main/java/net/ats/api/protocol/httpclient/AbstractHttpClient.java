...
public abstract class AbstractHttpClient {
	...
	public HttpResponse sendRequest(HttpUriRequest request, CookieStore cs) throws ClientProtocolException, IOException{
		...
		if(null != cs){
			domain = request.getURI().getHost();
			HttpClientBuilder builder = HttpClients.custom().setDefaultCookieStore(cs);//利用LaxRedirectStrategy处理POST重定向问题
	...
	private static String unifyCRLF(String headerStr, String separator) {
		String result = headerStr.replaceAll(Consts.CRLF, separator);
		result = headerStr.replaceAll(Consts.LINE_SEPARATOR, separator);
		result = headerStr.replaceAll(Consts.CR, separator);
		result = headerStr.replaceAll(Consts.LF, separator);
		return result;
	}
